...
import bson
from bson.binary import Binary  # <-- ADDED: Import Binary for MongoDB photo parsing
...
class MainApplication:
    ...
    def load_known_faces_from_db(self):
        ...
        CRITICAL UPDATE: Now only loads patients with status 'admitted'.
        """
        print("Loading known faces for admitted patients from database...")

        # Use the updated specialized method from DatabaseManager
        admitted_patients = self.db_manager.get_admitted_patients_with_photos()

        # Clear existing encodings to ensure we don't have stale data
        self.app_state["known_face_encodings"] = []
        self.app_state["known_face_names"] = []

        for patient in admitted_patients:
            patient_name = patient.get('name')
            photo_data = patient.get('photo')

            # --- FIX APPLIED HERE: Check for both bytes and bson.binary.Binary ---
            if isinstance(photo_data, (bytes, Binary)):
                try:
                    ...
                    if encodings:
                        ...
                        logging.warning(f"No face found in photo for admitted patient {patient_name}")
                except Exception as e:
                    logging.error(f"Could not process photo for {patient_name}: {e}")

        print(f"Successfully loaded {len(self.app_state['known_face_names'])} admitted patients into face recognition.")

        # Sync with core processor
